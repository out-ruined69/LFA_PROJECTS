...
while i< len(lines):
    ...
    if "Transitions" in lines[i]:
        ...
        while "End" not in lines[i]:
            ...
            i+=1
    i+=1
matriceAdiacentaAFD=[[[] for x in range(len(alfabet))] for y in range(len(stari))]
for i in listaTranzitii:
    matriceAdiacentaAFD[int(i[0])][alfabet.index(i[1])].append(int(i[2]))
lista = []
lista.append(start[0])
cuvant="abab"
for litera in cuvant:
    lista.append(matriceAdiacentaAFD[lista[len(lista)-1]][alfabet.index(litera)][0])
print(lista)
